...
Run **Google diagnostics** first. If all checks pass but the GIS popup still closes before returning a token, FinMate v2.6 includes an optional **Full-page Google fallback**. This legacy browser compatibility flow requires adding the exact FinMate page URL (for the GitHub Pages root, `https://sivarakesh89.github.io/`) to the OAuth client's **Authorized redirect URIs**. The access token is used only in memory and is not stored by FinMate.
